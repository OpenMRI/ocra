...
int main(int argc, char *argv[])
{
	...
	if(argc != 3) {
		...
		fprintf(stderr,"e.g.\t./mri_lab_vn_tests 60 32200\n");
		return -1;
	}
	...
	while(1) {
		...
		case 1: 
			...
			while(1) {
				...
				for(i = 0; i < 10; ++i) {
					while(*rx_cntr < 10000) usleep(50);
					// does the HDL core have a
					// FIFO which is being read
					// here? (Presumably a 64-bit
					// read from *rx_data)
		...
		case 2: 
			...
			while(1) {
				...
				if ( trig == 1 ) { // Change center frequency
					value = command & 0xfffffff;
					*rx_freq = (uint32_t)floor(value / FPGA_CLK_FREQ_HZ * (1<<30) + 0.5);
					printf("Setting frequency to %.4f MHz\n",*rx_freq * FPGA_CLK_FREQ_HZ / 1e6 / (1<<30) );
					if(value < 0 || value > 60000000) {
						printf("Frequency value out of range\n");
						continue;
					}          
					...
					printf("Socket Sending Error; saw trig 0x%lx.\n", trig);
				}
				...
				// transfer 10 * 5k = 50k samples
				for(i = 0; i < 10; ++i) {
					while(*rx_cntr < 10000) usleep(500); // wait until RX counter has acquired data
					for(j = 0; j < 5000; ++j) buffer[j] = *rx_data;
					send(sock_client, buffer, 5000*8, MSG_NOSIGNAL | (i<9?MSG_MORE:0));
				}
				printf("stop !!\n");
