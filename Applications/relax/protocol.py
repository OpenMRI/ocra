...
from PyQt5.QtWidgets import QApplication, QWidget, QTableWidget, QTableWidgetItem, QDesktopWidget
...
class ProtocolWidget(Protocol_Base, Protocol_Form):
    ...
    def __init__(self, parent=None):
        ...
        self.data = data()
        self.protocolCC = CCProtocolWidget()
        ...
        while self.cmdList != []: # while cmd list is not empty
            ...
            # Pause, Temperature, Change Sample
                ...
                if self.cmd == self.cmdElements[2]:
                    print('Pause.')
                    pause = int(self.protocol.item(0,1).text())
                    logger.add('PAUSE', dur=pause)
                    self.popup.show()
                    self.popup.set(pause, 'Pause protocol execution for {} seconds.'.format(pause), progress=True, btnEnable=False)
                    break
                # Change Temperature
                    ...
                    logger.add('TEMP')
                    self.popup.show()
                    self.popup.set(-1, 'Set the chiller temperature and confirm!')
                    break
                # Sample Exchange
                    ...
                    logger.add('CHNG')
                    self.popup.show()
                    self.popup.set(-1, 'Change the sample and confirmed!')
                    break
    ...
    def estimate_duration(self):
        ...
        for n in range(len(self.cmdList)):
            ...
            if cmd == self.cmdElements[0] or cmd == self.cmdElements[1]:
                start = int(self.protocol.item(n,1).text())
                stop = int(self.protocol.item(n,3).text())
                steps = int(self.protocol.item(n,5).text())
                tVals = np.rint(np.logspace(np.log10(start), np.log10(stop), steps))
                recovery = int(self.protocol.item(n,7).text())
                if int(self.protocol.item(n,9).text()) < 1 or \
                    int(self.protocol.item(n,11).text()) < 1: avgm = 1; avgp = 1
                else:
                    avgm = int(self.protocol.item(n,9).text())
                    avgp = int(self.protocol.item(n,11).text())
                progressValues.append(2*(sum(tVals)+len(tVals)*recovery)*avgp*avgp/1000)
            elif cmd == self.cmdElements[2]: progressValues.append(int(self.protocol.item(n,1).text()))
...
class protocol_popup(Popup_Dialog_Base, Popup_Dialog_Form):
    ...
    def set(self, dur, text, **kwargs):
        ...
        btnEnable = kwargs.get('btnEnable', True)

        self.popup_label.setText(text)
        self.progressWidget.setVisible(progFlag)
        self.popup_btn.setVisible(btnEnable)
